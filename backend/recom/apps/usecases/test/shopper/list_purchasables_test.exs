defmodule Usecases.Shopper.ListPurchasablesTest do
  ...
  test "given multiple purchasables are availalbe when retrieved then they are sorted by start date and returned" do
    instant = Timex.now()
    ...
    Mox.stub(PurchasablesGateway.MockAdapter, :all, fn _instant -> {:ok, found_purchasables} end)
    assert {:ok, [soon_purchasable, later_purchasable]} == list_purchasables(instant, PurchasablesGateway.MockAdapter)
  end
